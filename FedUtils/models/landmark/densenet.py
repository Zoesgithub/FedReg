...
class DenseNetModel(nn.Module):
    def __init__(self, optimizer, seed=1, withclassifier=True):
        super(DenseNetModel, self).__init__()
        self.num_classes = 2028
        self.num_inp = 64*64*3
        torch.manual_seed(123+seed)

        self.net = densenet121(num_classes=self.num_classes, withclassifier=withclassifier)  # DenseNet(num_classes=2)#FixupResNet(FixupBasicBlock, [4,4,4,4], 2)
    ...
    def solve_inner(self, data, num_epochs=1, extra_loss=None, step_func=None):
        ...
        if step_func:
            ...
            for _ in range(num_epochs):
                ...
                Loss=0
                for x, y in data:
                    loss=self.train_onestep([x, y], extra_loss)
        ...
        for d in data:
            ...
            with torch.no_grad():
                if self.cuda:
                    ...
                    y=y.cuda()
                pred = self.forward(x)
            loss += self.__loss(pred, y).sum().item()
            pred_max = pred.argmax(-1).float()
            assert len(pred_max.shape) == len(y.shape)
            tot_correct += (pred_max == y).float().sum().item()
